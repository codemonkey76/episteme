...
<template>
  <div class="flex flex-col h-full bg-bg overflow-hidden">
    ...
    <div class="flex items-center gap-2 px-3 py-2 border-b border-[#1e1e1e] shrink-0 flex-wrap">
      <select v-model="statusFilter" class="bg-surface text-[#c0c0c0] border border-raised rounded px-2 py-1 text-xs font-[inherit] cursor-pointer">
      ...
      <div class="flex items-center gap-[0.3rem] bg-surface border border-raised rounded px-2 py-[0.2rem] flex-1 min-w-[10rem] text-[#484848]">
        ...
        <input v-model="searchQuery" class="flex-1 bg-none border-none text-[#c0c0c0] text-xs font-[inherit] outline-none min-w-0 placeholder:text-[#404040]" placeholder="Search tasks…" />
    ...
    <div v-if="adding" class="flex flex-col gap-2 px-3 py-2.5 border-b border-[#1e1e1e] bg-[#111] shrink-0">
      <input v-model="draft.title" class="bg-surface text-fg border border-raised rounded px-2 py-1.5 text-[0.8125rem] font-[inherit] outline-none focus:border-[#3a6adf] placeholder:text-[#404040]" placeholder="What needs doing?" @keyup.enter="saveDraft" />
      <textarea v-model="draft.notes" rows="2" class="bg-surface text-fg border border-raised rounded px-2 py-1.5 text-[0.8125rem] font-[inherit] outline-none resize-y focus:border-[#3a6adf] placeholder:text-[#404040]" placeholder="Notes (optional)" />
      <div class="flex items-center gap-2 flex-wrap">
        <input v-model="draft.due" type="datetime-local" class="bg-surface text-[#c0c0c0] border border-raised rounded px-2 py-1 text-xs font-[inherit] cursor-pointer [color-scheme:dark]" />
        <select v-model="draft.priority" class="bg-surface text-[#c0c0c0] border border-raised rounded px-2 py-1 text-xs font-[inherit] cursor-pointer">
          <option v-for="p in PRIORITIES" :key="p" :value="p">{{ p }}</option>
        </select>
        <button class="bg-[#1e3a6e] text-[#7ab0ff] border border-[#2a4a8a] rounded px-3 py-1 text-xs font-[inherit] cursor-pointer transition-colors duration-100 hover:not-disabled:bg-[#254880] disabled:opacity-50" :disabled="!draft.title.trim()" @click="saveDraft">Save</button>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-else>
        <div v-for="t in filtered" :key="t.id" class="group flex items-start gap-3 px-3.5 py-2.5 border-b border-[#161616] hover:bg-[#131313]">
          <!-- Edit mode -->
          <div v-if="editingId === t.id" class="flex-1 flex flex-col gap-2 min-w-0">
            <input v-model="draft.title" class="bg-surface text-fg border border-raised rounded px-2 py-1.5 text-[0.8125rem] font-[inherit] outline-none focus:border-[#3a6adf]" @keyup.enter="saveDraft" />
            <textarea v-model="draft.notes" rows="2" class="bg-surface text-fg border border-raised rounded px-2 py-1.5 text-[0.8125rem] font-[inherit] outline-none resize-y focus:border-[#3a6adf]" placeholder="Notes (optional)" />
            <div class="flex items-center gap-2 flex-wrap">
              <input v-model="draft.due" type="datetime-local" class="bg-surface text-[#c0c0c0] border border-raised rounded px-2 py-1 text-xs font-[inherit] cursor-pointer [color-scheme:dark]" />
              <select v-model="draft.priority" class="bg-surface text-[#c0c0c0] border border-raised rounded px-2 py-1 text-xs font-[inherit] cursor-pointer">
                <option v-for="p in PRIORITIES" :key="p" :value="p">{{ p }}</option>
          ...
          <template v-else>
            <button
              ...
              :class="t.status === 'done' ? 'border-[#4caf6e] text-[#4caf6e]' : 'border-[#404040] text-transparent hover:border-[#4caf6e]'"
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[0.8125rem] leading-[1.45] break-words" :class="t.status === 'done' ? 'text-[#585858] line-through' : 'text-[#d0d0d0]'">{{ t.title }}</p>
              <p v-if="t.notes" class="text-[0.75rem] text-[#707070] leading-[1.4] break-words mt-[0.1rem]">{{ t.notes }}</p>
              <div class="text-[0.68rem] mt-[0.2rem] flex items-center gap-1.5">
                ...
                <template v-if="t.due_at">
                  ...
                  <span :class="isOverdue(t) ? 'text-[#ff7070]' : 'text-[#505050]'">due {{ fmtDue(t.due_at) }}</span>
                </template>
              </div>
            </div>
            <div class="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-100">
              <button class="text-[#606060] hover:text-[#a0c0ff] p-1 cursor-pointer bg-none border-none" title="Edit" @click="startEdit(t)">
              ...
              <button class="text-[#606060] hover:text-[#d08080] p-1 cursor-pointer bg-none border-none" title="Delete" @click="remove(t)">
