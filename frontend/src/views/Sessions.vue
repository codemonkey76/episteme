...
<template>
  <div class="p-5 max-w-[40rem]">
    <div class="flex justify-between items-center mb-4">
      ...
      <button @click="newSession" class="bg-[var(--c-2a4a7a)] text-fg border-none rounded-md py-1.5 px-3 cursor-pointer text-[0.85rem]">New</button>
    </div>
    <ul class="list-none flex flex-col gap-2">
      <li v-for="s in store.sessions" :key="s.id" class="flex items-center gap-3 bg-surface rounded-md py-[0.6rem] px-[0.8rem]">
        ...
        <span class="text-xs text-[var(--c-606060)]">{{ new Date(s.updated_at).toLocaleDateString() }}</span>
        <button class="bg-none border-none text-[var(--c-606060)] cursor-pointer text-xs" @click="remove(s.id)">✕</button>
      </li>
    </ul>
  </div>
</template>
