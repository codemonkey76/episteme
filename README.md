...
- **Memory consolidation ("dreaming")** — a sleep-like pass reviews the memory store to merge redundant memories, resolve conflicts (preferring the newer/more-specific), and synthesise higher-order **lessons** that generalise beyond any single memory. Runs nightly while idle (skipped unless enough new memories have accrued) or on demand via the **Memories** window's **Dream** button — with a model picker so you can point it at your smartest model (Sonnet/Opus) rather than the local one. Changes auto-apply but are **reversible**: deletes are soft, every change lands in the Logs, and the Memories **Archive** restores anything
...
- **GitHub (read-only)** — connect a personal access token in **Settings → Integrations** and the agent can look at commits, files, and pull requests across your repos (`github_get_commit`, `github_list_commits`, `github_get_file`, …) and cite them — e.g. pull a fix's diff and reference the commit URL in a helpdesk reply. Read-only by design; set a default owner to refer to repos by bare name
...
- **Terminal** — tabbed bash/PowerShell shells (xterm.js over a real PTY in the container) with an **AI sidebar** that drives the shell: it **types each command into your prompt** for you to edit and run, then reads the output to decide the next step. Device-code sign-in for Microsoft 365 admin (Exchange Online). Selections auto-copy to the clipboard; PowerShell uses the classic white-on-blue scheme. Full scrollback is **persisted and searchable** across refreshes and server restarts — a refresh attaches a fresh shell (never resurrecting the old one) but repaints your prior history, and an **Archive** search spans every session over months
